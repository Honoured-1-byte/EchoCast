...
export default function TerminalVisualizer({ config, currentTheme, speakingPersona, isPlaying }) {
    ...
    useEffect(() => {
        ...
        const init = () => {
            particles = [];
            const count = config.vibe === 'neon' ? 100 : (config.vibe === 'cool' ? 200 : 60);
        ...
        const render = () => {
            ...
            ctx.fillStyle = config.vibe === 'neon' ? 'rgba(0, 0, 0, 0.1)' : currentTheme.surface.replace('0.9', '0.2');
    ...
    }, [currentTheme, speakingPersona, isPlaying, config.vibe]);
